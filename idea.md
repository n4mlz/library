...
- 木は必ず二部グラフ
- [高難易度木問題を解くテクニック集](https://speakerdeck.com/tatyam_prime/gao-nan-yi-du-mu-wen-ti-wojie-kutekunitukuji?slide=50)
...
## 最小全域木(MST)
- グラフが連結であるという条件を保ったまま辺を消去して得られる木
- クラスカル法とUnion-Findで求められる

## クラスカル法
- 最小全域木を求めるアルゴリズム
- グラフの辺をコストが小さい順に閉路を作らないように採用していく貪欲法
- 計算量はO(Elog(V))
...
- 任意の2頂点の対に対して最短経路を求める問題
    ...
    - (未検索だがのちに必要になった時用のメモ)
        - グラフが木であればLCA(最小共通祖先)を使った方法を使えば前計算をO(V)、クエリごとにO(logV)で解けるらしい？
        - [参考](https://qiita.com/ageprocpp/items/cdf67e828e1b09316f6e)
...
## オイラーツアー
- 木の根からDFSの探索で訪れた頂点を、その順に記録したもの
- 任意の連続する部分列に含まれる頂点集合が部分木になる
- LCAを効率的に求めるときに使う

## 木上での全頂点対最短経路問題

- (木上の2頂点u, vの距離) = (根からuまでの距離)+(根からvまでの距離)-2*(根からu, vのLCAまでの距離)
- LCA(最小共通祖先)はオイラーツアーとセグメント木で高速に求められる
    - 前処理 O(V)
    - クエリごと O(log(V))
- [参考](https://qiita.com/ageprocpp/items/cdf67e828e1b09316f6e)
...
- 「～となる〇〇の最大値」を値に持つDPは適当な添字と入れ替えて「～となる××の最小値」というDPにできることが多い
...
- 木に新しい辺を追加するとき、閉路ができるか？を知れる
    - uとvが連結のときuとvのグループが同じ、とする
    - 追加しようとした辺の両端が既に同じグループならその辺を追加すると閉路ができる
...
- 以下を O(log n) で行える
    ...
    - 要素の削除
    - x以上の最小の要素の検索
- setとheapqの良いとこ取りをしたような機能
    - setなら順序がないのでxが存在するかどうかはわかるがx以上の最小の要素の検索などは O(n)
    - heapqなら全体で見た最小はわかるがx以上で最小などの検索は難しい、また要素の削除も先頭しか行えない
- [参考](https://qiita.com/tatyam/items/492c70ac4c955c055602)

## セグメント木
- 区間を扱うのに適したデータ構造
- RMQ(Range Minimum Queries)が解ける
- [参考](https://algo-logic.info/segment-tree/)
- [参考](https://qiita.com/takayg1/items/c811bd07c21923d7ec69)

## HL分解
- なんか変更に強い木の持ち方らしい(未検索)
...
- 複数回操作を行う問題の場合
    ...
    - 同じ操作を何度もしてないかを考える(計算量削減)
    - 操作と遷移をグラフにする(状態数が少ない場合など)
        - 状態を頂点、遷移を辺としてbfsやdfsをしてみる
- クエリに答える系
    - クエリ並列化ができないか考えるV(全てのクエリを同時に考える)
- グラフに置き換える
- 同じ意味のものをまとめて考える
